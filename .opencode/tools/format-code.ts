 ...
 * Supports Dart (flutter format), Biome, Prettier, Black, gofmt, rustfmt.
 */
...
type Formatter = "dart" | "biome" | "prettier" | "black" | "gofmt" | "rustfmt"

const formatCodeTool: ToolDefinition = tool({
  description:
    "Detect formatter for a file and return the exact command to run (Dart, Biome, Prettier, Black, gofmt, rustfmt).",
  args: {
    filePath: tool.schema.string().describe("Path to the file to format"),
    formatter: tool.schema
      .enum(["dart", "biome", "prettier", "black", "gofmt", "rustfmt"])
      .optional()
