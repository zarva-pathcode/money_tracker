 ...
 * Supports Dart/Flutter (primary), Biome, ESLint, Ruff, pylint, golangci-lint.
 */
...
type Linter = "dart" | "biome" | "eslint" | "ruff" | "pylint" | "golangci-lint"
...
const lintCheckTool: ToolDefinition = tool({
  ...
  args: {
    ...
    linter: tool.schema
      .enum(["dart", "biome", "eslint", "ruff", "pylint", "golangci-lint"])
      .optional()
...
function detectLinter(cwd: string): Linter {
  // Dart/Flutter project detection (priority for Flutter projects)
...
function buildLintCommand(linter: Linter, target: string, fix: boolean): string {
  if (linter === "dart") return `dart analyze${fix ? " --fatal-infos" : ""} ${target}`
