 ...
 * Supports Flutter/Dart (pub outdated), npm audit, secret scanning, and OWASP checks.
 *
  ...
  async execute(args, context) {
    ...
    if (auditType === "all" || auditType === "dependencies") {
      results.checks.push({
        ...
        command: isFlutter ? "dart pub outdated" : "npm audit",
    ...
    results.recommendations = generateRecommendations(results, isFlutter)
...
async function scanForSecrets(
  ...
  const srcDir = path.join(cwd, "lib")
  if (fs.existsSync(srcDir)) {
    await scanDirectory(srcDir, secretPatterns, ignorePatterns, findings)
  }

  const srcAltDir = path.join(cwd, "src")
  if (fs.existsSync(srcAltDir)) {
    await scanDirectory(srcAltDir, secretPatterns, ignorePatterns, findings)
  }

  // Also check root config files
  const configFiles = ["config.dart", "settings.dart", "config.js", "config.ts"]
...
async function scanDirectory(
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && entry.name.match(/\.(dart|ts|tsx|js|jsx|json)$/)) {
...
async function scanCodeSecurity(
  ...
  const srcDir = path.join(cwd, "lib")
  if (fs.existsSync(srcDir)) {
    await scanDirectory(srcDir, securityPatterns, ["node_modules", ".git", "dist", ".dart_tool"], findings)
  }

  const srcAltDir = path.join(cwd, "src")
  if (fs.existsSync(srcAltDir)) {
    await scanDirectory(srcAltDir, securityPatterns, ["node_modules", ".git", "dist", ".dart_tool"], findings)
  }

  return findings
}

function generateRecommendations(results: AuditResults, isFlutter: boolean): string[] {
  ...
  for (const check of results.checks) {
    if (check.status === "failed" && check.name === "Secret Detection") {
      ...
      recommendations.push("Use flutter_dotenv or similar package for production")
    }
    ...
    if (check.status === "warning" && check.name === "Code Security") {
      ...
      recommendations.push("Use Dart's built-in HTML sanitization where needed")
    }

    if (check.status === "pending" && check.name === "Dependency Vulnerabilities") {
      if (isFlutter) {
        recommendations.push("Run 'dart pub outdated' to check for outdated dependencies")
        recommendations.push("Review pubspec.yaml for dependency security")
      } else {
