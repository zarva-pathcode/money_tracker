 ...
 * Supports Flutter/Dart (lcov.info), Istanbul/nyc coverage report formats.
 */
...
const checkCoverageTool: ToolDefinition = tool({
  description:
    "Check test coverage against a threshold and identify files with low coverage. Reads coverage reports from common locations (lcov, Istanbul).",
  args: {
  ...
  async execute(args, context) {
    ...
    // Look for coverage reports — Flutter lcov.info first, then Istanbul/nyc
    const coveragePaths = [
      "coverage/lcov.info",
    ...
    for (const coveragePath of coveragePaths) {
      ...
      if (fs.existsSync(fullPath)) {
        if (coveragePath.endsWith("lcov.info")) {
          try {
            const content = fs.readFileSync(fullPath, "utf-8")
            coverageData = parseLcovInfo(content)
            coverageFile = coveragePath
            break
          } catch {
            // Continue to next file
          }
        } else if (coveragePath.endsWith(".json")) {
          try {
    ...
    if (!coverageData) {
      return JSON.stringify({
        ...
        suggestion:
          "Run tests with coverage first: flutter test --coverage",
...
function parseLcovInfo(lcov: string): CoverageSummary {
  const lines: string[] = lcov.split("\n")
    ...
    if (line.startsWith("SF:")) {
      ...
      if (currentFile) {
        files.push({
          ...
          percentage: totalLines > 0 ? (totalCovered / totalLines) * 100 : 100,
        })
      }
      currentFile = null
      totalLines = 0
  ...
  return {
    total: {
      lines: grandTotal.lines,
      covered: grandTotal.covered,
      percentage: grandTotal.lines > 0 ? (grandTotal.covered / grandTotal.lines) * 100 : 0,
    },
    files,
  }
}
